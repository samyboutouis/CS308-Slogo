...
### Main Problem:
Designing a flexible SLogo IDE that allows users to interactively control a visualization frontEndTurtle by inputting unique commands.
...
#### Back-end External:
Frontend will either call these methods itself or delgate to a controller, but either way the back-end external methods will provide the front end with a list of new coordinates to go to and whether the pen was up or down during those frontEndTurtle movements
#### Back-end Internal:
...
```
Implementations:
* The frontEndTurtle class encapsulates our frontEndTurtle sprite, so in case we decide to change the implementation from an ImageView to a Polygon, no other piece of code should have to change much
#### Back-end External:
```java
...
1. Turtle Display (UI screen where the frontEndTurtle can move & draw)
...
6. Toolbar (UI component that holds different UI buttons to allow the user to change certain visual parameters)
    ...
    - Button to set an image to use for the frontEndTurtle
...
The following errors should be reported back to the user and NOT cause the program to crash:
  - Bad input data
      ...
      - incorrect image uploaded for frontEndTurtle icon
  - Empty data
...
#### Front-end Internal:
1. Supports drawing the frontEndTurtle in the correct locations and with or without the pen
    * Uses Movements from backend to display frontEndTurtle for each change in direction/position
2. Supports visual UI changes by calling delegate methods within itself to change color or appearance of view
3. Multiple classes in the front end internal API for representing the frontEndTurtle, windows, etc.
...
2. Supports the sending of Movements specifying new frontEndTurtle position, orientation, and pen status from the back-end to the front-end
    * By creating a class to represent all the potential movements of the frontEndTurtle and pen, it can be easily extended to incorporate different types of movement such as size change or color change
3. All of the commands and values the used typed will be encapsulated by the Movement object, so the front end does not care about the commands, only where to move the frontEndTurtle
...
2. Perhaps another class or method used to break down command into the resulting movement object (representing the new position, orientation, pen up/down of the frontEndTurtle)
...
1. Interpret an initial set of SLogo commands
    ...
    3. Correct command:
        - Expected outcome: program should parse the data and move the frontEndTurtle as expected from the SLogo command in the visualization window
        - How design supports testing: the appropriate command should move the frontEndTurtle in the view as expected (e.g., "fd 10" should move the frontEndTurtle in the view forward by 10 steps)

2. Setting a background color for the frontEndTurtle's display area
...
3. Set image for the frontEndTurtle to use
    1. Upload standard frontEndTurtle image:
        - Expected outcome: Once a user selects an image to represent the frontEndTurtle, the frontEndTurtle on the screen should be the selected image
    ...
    2. Cancel upload:
        ...
        - How design supports testing: one can compare the UI display change with the expected change (no change in frontEndTurtle image)
...
4. See history of commands
    ...
    2. User types in an invalid command
        * Expected outcome: The frontEndTurtle does not change position/direction and no changes are seen in the history window
    ...
    3. User types in a few valid commands, and then an invalid command
        * Expected outcome: the frontEndTurtle does not move, and no change is shown in the history window
...
1.  One major design decision was to make the back-end return a complete list of all movements that the frontEndTurtle goes through in one complete command cycle. For instance, if the user-inputted command was a for loop of making the frontEndTurtle move forward and rotate for each step, the back-end would return a list of all the movements that the frontEndTurtle takes before the loop ends. The front-end could then use this list to convert it into actual movement of the visualization frontEndTurtle in the view.

    One alternative design that was initially discussed was to have the front-end call the back-end for every single step in the command loop. The back-end would then return the new movement object (formatted as described previously), and the front-end would use this new movement to move the visuaslization frontEndTurtle.

    The team decided that the former was a better design because it was more versatile for different possible features. For instance, having a list of the complete set of movements that the frontEndTurtle would have to take could allow for easier implementation of features regarding time, such as allowing the user to step through each move or animating the frontEndTurtle instead of having it complete the entire command in one step.

2. Another design decision discussed is whether the front end should pass in the current location and orientation of the frontEndTurtle or the back end should keep a variable representing the current location/position
    ...
    * We decided to have the front end pass in the current location of the frontEndTurtle for each command, so in case a new feature is allowed the user to position the frontEndTurtle without a command, our code would not need any additional API calls to let the back end know the position changed
